...
第3章は正規化の手順（1NF → 2NF → 3NF）が中心テーマ。書籍を読めば手順自体は理解できるが、実際に非正規形のデータを目の前にして「どの関数従属性を見つけるか」「どこで分割するか」を自分で判断できるかは別の話。そこで、独自の題材（オンライン書店の注文管理）の非正規形テーブルを提示し、段階的に正規化を自分の手で進める形式にした。各正規形に至る過程で「なぜこの分割が必要か」を考えることが狙い。
