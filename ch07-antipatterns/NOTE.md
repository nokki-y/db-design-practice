...
第7章のテーマは「やってはいけない」論理設計のアンチパターン。書籍では8つのアンチパターンが個別に解説されるが、実務では複数のアンチパターンが1つのシステムに混在していることが多い。そこで、架空のオンライン学習プラットフォーム「LearnHub」の設計に5つのアンチパターンを仕込み、既存設計のレビューとして「問題を見つけ → 名前をつけ → 修正優先度を判断する」形式にした。単にパターン名を知っているだけでなく、実際のテーブル定義から問題を嗅ぎ取る力を養うことが狙い。
